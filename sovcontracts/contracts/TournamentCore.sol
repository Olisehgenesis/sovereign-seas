...
interface ISovereignSeas {
    function getCampaignProjects(uint256 campaignId) external view returns (uint256[] memory);
...
contract TournamentCore is ReentrancyGuard, Pausable {
    ...
    function _createTournament(
        ...
        // Only fetch projects if campaign ID is provided
        if (_sovseasCampaignId > 0) {
            uint256[] memory projectIds = sovseas.getCampaignProjects(_sovseasCampaignId);
    ...
    function addCampaignProjects(uint256 _tournamentId, uint256 _campaignId) external {
        ...
        uint256[] memory projectIds = sovseas.getCampaignProjects(_campaignId);
        uint256 added = 0;
