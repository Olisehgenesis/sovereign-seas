...
async function main() {
  ...
  let contractAddress = process.env.SEAS_CONTRACT_ADDRESS || process.env.CONTRACT_ADDRESS;
  ...
  if (!contractAddress) {
    ...
    console.error("     CONTRACT_ADDRESS=0x... pnpm run verify:seas:celo");
    console.error("  2. Or use the script with address:");
    console.error("     CONTRACT_ADDRESS=0x... pnpm run verify:seas:celo");
    console.error("\nExample:");
    console.error("  CONTRACT_ADDRESS=0x... pnpm run verify:seas:celo-sepolia");
    process.exit(1);
  }
