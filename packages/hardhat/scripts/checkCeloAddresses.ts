...
const CELO_MAINNET_ADDRESSES = {
  ...
  UBESWAP_V2_ROUTER: '0xE3D8bd6Aed4F5bc0a19D39c3e1eAbE9f8D018520', // Ubeswap V2 Router on Celo
...
const ALFAJORES_ADDRESSES = {
  ...
  UBESWAP_V2_ROUTER: '0xE3D8bd6Aed4F5bc0a19D39c3e1eAbE9f8D018520', // Ubeswap V2 Router on Alfajores (same as mainnet)
...
async function checkAddresses() {
  ...
  console.log(`UBESWAP_V2_ROUTER_ADDRESS: ${process.env.UBESWAP_V2_ROUTER_ADDRESS || 'NOT SET'}`);
  console.log(`CELO_UNISWAP_V3_VOTING_PROXY_ADDRESS: ${process.env.CELO_UNISWAP_V3_VOTING_PROXY_ADDRESS || 'NOT SET'}`);
  console.log(`UBESWAP_VOTING_PROXY_ADDRESS: ${process.env.UBESWAP_VOTING_PROXY_ADDRESS || 'NOT SET'}`);
  ...
  console.log(`UBESWAP_V2_ROUTER_ADDRESS=${CELO_MAINNET_ADDRESSES.UBESWAP_V2_ROUTER}`);
  ...
  console.log(`UBESWAP_V2_ROUTER_ADDRESS=${ALFAJORES_ADDRESSES.UBESWAP_V2_ROUTER}`);
  ...
  if (!process.env.UBESWAP_V2_ROUTER_ADDRESS) {
    console.log('❌ UBESWAP_V2_ROUTER_ADDRESS is not set');
  }
  ...
  console.log('4. For Uniswap: Run: pnpm run deploy:uniswap-proxy:celo');
  console.log('5. For Ubeswap: Run: pnpm run deploy:ubeswap-proxy:celo');
}
