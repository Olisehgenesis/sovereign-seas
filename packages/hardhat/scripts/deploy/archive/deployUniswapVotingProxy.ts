...
try {
  contractBytecode = uniswapVotingProxyAbi.bytecode;
...
async function deployUniswapVotingProxyInternal() {
  try {
    ...
    let abi = uniswapVotingProxyAbi.abi;
    ...
    const hash = await walletClient.deployContract({
      abi: uniswapVotingProxyAbi.abi,
      bytecode: contractBytecode as `0x${string}`,
      args: [
        UNISWAP_V2_ROUTER_ADDRESS as `0x${string}`,
    ...
    console.log('');
    console.log('Add this address to your .env file as UNISWAP_VOTING_PROXY_ADDRESS to use it with your application.');
    console.log('');
    console.log('Constructor arguments:');
    console.log(`- Uniswap V2 Router: ${UNISWAP_V2_ROUTER_ADDRESS}`);
