 ...
 */
contract SovereignSeas is Ownable, ReentrancyGuard {
    ...
    event VoteCast(address indexed voter, uint256 indexed campaignId, uint256 indexed projectId, uint256 amount, uint256 voteCount);
    ...
    function submitProject(
        ...
        campaignProjects[_campaignId].push(Project({
            id: projectId,
    ...
    function distributeFunds(uint256 _campaignId) external nonReentrant {
        require(_campaignId < campaigns.length, "Campaign does not exist");
        Campaign storage campaign = campaigns[_campaignId];
        require(msg.sender == campaign.admin || msg.sender == owner(), "Only campaign admin or platform owner can distribute");
     ...
     */
    function getUserVotesForProject(uint256 _campaignId, address _user, uint256 _projectId) external view returns (uint256) {
