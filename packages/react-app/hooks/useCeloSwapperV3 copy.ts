...
const swapAndVoteToken = async (
    ...
    try {
      ...
      const allowance = await publicClient.readContract({
        ...
        args: [walletAddress as `0x${string}`, actualContractAddress],
      }) as bigint;
      ...
      console.log("Executing swap and vote with args:", {
        token,
        campaignId: BigInt(campaignId).toString(),
        projectId: BigInt(projectId).toString(),
        amountBigInt: amountBigInt.toString(),
        minCeloAmount: minCeloAmount.toString()
      });
      
      // Execute swap and vote with proper error handling
    ...
    //   if (!swapTx) {
    //     console.error("Swap transaction failed or returned undefined");
    //     throw new Error("Swap transaction failed");
    //   }
      
    //   console.log("Swap tx hash:", swapTx);
