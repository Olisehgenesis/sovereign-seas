...
import { IdentitySDK } from '@/utils/useGooddollar';
...
const VerifyModal = ({ isOpen, onClose, onVerificationComplete }) => {
 ...
 const startFaceVerification = async () => {
   if (!identitySDK) {
     setError('IdentitySDK not initialized. Please ensure your wallet is connected.');
     return;
   }
   
   if (typeof window === 'undefined') {
     setError('Window is not available. Please try again.');
     return;
   }
   ...
   try {
     ...
     if (!popup) {
       setError('Failed to open popup window. Please check your browser settings.');
       setIsLoading(false);
       return;
     }
